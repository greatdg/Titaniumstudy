...
var _ = __p.require("alloy/underscore")._, Backbone = __p.require("alloy/backbone"), CONST = __p.require("alloy/constants");
...
exports.M = function(name, modelDesc, migrations) {
    ...
    var mod;
    if (adapter.type) {
        mod = __p.require("alloy/sync/" + adapter.type);
        extendObj.sync = function(method, model, opts) {
            mod.sync(method, model, opts);
        };
    } else extendObj.sync = function(method, model) {
        __log.warn("Execution of " + method + "#sync() function on a model that does not support persistence");
        __log.warn("model: " + JSON.stringify(model.toJSON()));
    };
...
exports.C = function(name, modelDesc, model) {
    ...
    if (config.adapter && config.adapter.type) {
        mod = __p.require("alloy/sync/" + config.adapter.type);
        extendObj.sync = function(method, model, opts) {
            mod.sync(method, model, opts);
        };
    } else extendObj.sync = function(method, model) {
        __log.warn("Execution of " + method + "#sync() function on a collection that does not support persistence");
        __log.warn("model: " + JSON.stringify(model.toJSON()));
    };
...
exports.createStyle = function(controller, opts, defaults) {
    ...
    var styleArray;
    styleArray = controller && _.isObject(controller) ? __p.require("alloy/widgets/" + controller.widgetId + "/styles/" + controller.name) : __p.require("alloy/styles/" + controller);
    var styleFinal = {};
    var i, len;
...
exports.createWidget = function(id, name, args) {
    ...
    return new (__p.require("alloy/widgets/" + id + "/controllers/" + (name || DEFAULT_WIDGET)))(args);
};

exports.createController = function(name, args) {
    var newController = new (__p.require("alloy/controllers/" + name))(args);
...
exports.createModel = function(name, args) {
    return new (__p.require("alloy/models/" + ucfirst(name)).Model)(args);
};

exports.createCollection = function(name, args) {
    return new (__p.require("alloy/models/" + ucfirst(name)).Collection)(args);
};
...
exports.CFG = __p.require("alloy/CFG");