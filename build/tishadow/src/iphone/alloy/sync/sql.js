...
function Migrator(config, transactionDb) {
    ...
    this.column = function(name) {
        ...
        switch (type.toLowerCase()) {
          ...
          case "date":
          case "datetime":
            __log.warn('"' + type + '" is not a valid sqlite field, using TEXT instead');
          ...
          case "bigint":
          case "boolean":
            __log.warn('"' + type + '" is not a valid sqlite field, using INTEGER instead');
          ...
          case "decimal":
          case "number":
            __log.warn('"' + name + '" is not a valid sqlite field, using REAL instead');
...
function Sync(method, model, opts) {
    ...
    switch (method) {
      ...
      case "update":
        resp = function() {
            ...
            if (null === model.id) {
                ...
                } else __log.warn("Unable to get ID from database for model: " + model.toJSON());
                rs && rs.close();
            }
            db.execute("COMMIT;");
            db.close();
      ...
      case "read":
        opts.query && opts.id && __log.warn('Both "query" and "id" options were specified for model.fetch(). "id" will be ignored.');
...
function installDatabase(config) {
    ...
    __log.debug('Installing sql database "' + dbFile + '" with name "' + dbName + '"');
    var db = Ti.Database.install(__p.file(dbFile), dbName);
    if (false === config.adapter.remoteBackup && true) {
        __log.debug('iCloud "do not backup" flag set for database "' + dbFile + '"');
    ...
    if (config.adapter.idAttribute) {
        ...
        __log.info('No config.adapter.idAttribute specified for table "' + table + '"');
        __log.info('Adding "' + ALLOY_ID_DEFAULT + '" to uniquely identify rows');
...
var _ = __p.require("alloy/underscore")._;
...
module.exports.beforeModelCreate = function(config, name) {
    ...
    __log.debug(Ti.Platform.osname);
    __log.debug(Ti.Database);
    __log.debug(typeof Ti.Database);
    ...
    if (!config.adapter.idAttribute) {
        __log.info('No config.adapter.idAttribute specified for table "' + config.adapter.collection_name + '"');
        __log.info('Adding "' + ALLOY_ID_DEFAULT + '" to uniquely identify rows');
        config.columns[ALLOY_ID_DEFAULT] = "TEXT UNIQUE";
        config.adapter.idAttribute = ALLOY_ID_DEFAULT;
    }
