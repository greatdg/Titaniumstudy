...
//Rare exceptions to indicate a bug in the _damate code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_damate = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_damate]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.damate.module"
